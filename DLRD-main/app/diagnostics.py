...
def sql_outcomes(parses) -> dict:
    ...
    fail_active: collections.Counter = collections.Counter()
    fail_commented: collections.Counter = collections.Counter()
    # File-level .sql found-vs-parsed gap, classified by reason (counts only).
    file_gap: collections.Counter = collections.Counter()
    for p in parses:
        ...
        fail_active.update({str(k): int(v) for k, v in (stats.get("fail_active") or {}).items()})
        fail_commented.update({str(k): int(v) for k, v in (stats.get("fail_commented") or {}).items()})
        # Explain the .sql file-level gap: a recognized .sql file that yielded no
        # objects/table refs/proc calls (parse_ok is False). Bucket the reason
        # from the same tally-only stats — no names, no SQL.
        if getattr(p, "language", None) == "sql" and not getattr(p, "parse_ok", False):
            file_gap["files_unparsed"] += 1
            attempted = int(stats.get("parsed", 0)) + int(stats.get("skipped", 0))
            active_failures = sum(int(v) for v in (stats.get("fail_active") or {}).values())
            if attempted == 0:
                file_gap["empty_after_preprocess"] += 1
            elif active_failures > 0:
                file_gap["all_statements_failed"] += 1
            else:
                file_gap["parsed_no_entities"] += 1
    return {
        ...
        "fail_categories": {
            "active": {k: int(v) for k, v in fail_active.items()},
            "commented": {k: int(v) for k, v in fail_commented.items()},
        },
        "file_gap": {k: int(v) for k, v in file_gap.items()},
        "recovery": {
    ...
    *,
    ...
    metrics = {
        ...
        "sql": {
            ...
            "fail_categories": sql_stats.get("fail_categories", {"active": {}, "commented": {}}),
            "file_gap": sql_stats.get("file_gap", {}),
        },
        "enrichment": {
