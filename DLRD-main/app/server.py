...
from .pipeline import GRAPH_PATH, PROJECT_CONTEXT_PATH, STORY_PATH, manager
...
@app.get("/project-context.json")
async def project_context() -> Response:
    # Sidecar document context (global narrative + per-node doc snippets). Fills
    # in after the graph; the dashboard tolerates its absence and reads it lazily.
    if PROJECT_CONTEXT_PATH.exists():
        return _serve_data_file(PROJECT_CONTEXT_PATH)
...
async def api_regenerate_story(force: bool = False) -> dict:
    ...
    if not s.is_configured():
        raise HTTPException(status_code=400, detail="Not configured. Save apiBase/apiKey/model first.")
    # Read the sidecar document context (if the doc phase has produced it) so a
    # regenerated story incorporates the global narrative. Best-effort, no egress.
    doc_context = None
    if PROJECT_CONTEXT_PATH.exists():
        try:
            doc_context = json.loads(PROJECT_CONTEXT_PATH.read_text(encoding="utf-8"))
        except Exception:
            doc_context = None
    llm = LLMClient(s)
    try:
        story = story_mod.generate_story(graph, llm, s, doc_context=doc_context)
    finally:
        llm.close()
...
async def api_chat(payload: dict) -> dict:
    """Graph-aware retrieval chat: retrieves context from the knowledge graph
    and answers a natural-language question via the configured internal LLM
    service.

    The only outbound network call goes through the existing guarded client
    (app/llm.py + app/http_guard.py) — no new egress path is introduced.
    """
    s = Settings.load()
    if not s.is_configured():
        raise HTTPException(
            status_code=400,
            detail="Not configured. Save apiBase/apiKey/model first.",
        )
    if not GRAPH_PATH.exists():
        raise HTTPException(
            status_code=404,
            detail="No knowledge graph yet. Run an analysis first.",
        )
    ...
    history = [
        ...
        for h in (raw_history if isinstance(raw_history, list) else [])
    ...
    llm = LLMClient(s)
    try:
        result = rag_mod.answer_question(question, history, llm, s)
    finally:
        llm.close()
...
async def api_reindex() -> dict:
    """Rebuild the local semantic embedding index from the existing knowledge graph.

    Semantic search is an optional, fully local capability. When the embedding
    backend is unavailable the chat keeps working with lexical retrieval, so this
    endpoint reports a clear disabled status instead of failing.
    """
    from . import embeddings as embed_mod

    if not embed_mod.EmbeddingIndex.is_available():
        return {
            "ok": False,
            "indexed": False,
            "message": "Semantic indexing is disabled; the chat uses lexical retrieval.",
        }
    if not GRAPH_PATH.exists():
        raise HTTPException(
            status_code=404, detail="No knowledge graph yet. Run an analysis first."
        )
    try:
        graph = json.loads(GRAPH_PATH.read_text(encoding="utf-8"))
    except Exception:
    ...
    index = embed_mod.EmbeddingIndex.build(nodes)
    if index is None:
        return {
            "ok": False,
            "indexed": False,
            "message": "Semantic indexing is unavailable; the chat uses lexical retrieval.",
        }
    info = embed_mod.EmbeddingIndex.index_info()
    return {"ok": True, "message": f"Index built for {info.get('node_count', 0)} nodes.", **info}
