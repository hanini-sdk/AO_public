...
async def api_chat(payload: dict) -> dict:
    """RAG chat: retrieves context from the knowledge graph and answers a
    natural-language question via the configured LLMAAS.
...
async def api_reindex() -> dict:
    """Rebuild the local embedding index from the existing knowledge graph.

    Allows re-indexing a project without re-running the full analysis pipeline
    (useful when fastembed was installed after the last analysis, or when the
    model cache was seeded on the work machine after the fact).

    Returns the index metadata on success, or an error message if fastembed is
    not available or the graph is missing.
    """
    ...
    if not embed_mod.EmbeddingIndex.is_available():
        return {
            ...
            "message": (
                ...
                "then pre-seed the model cache (see README / embeddings.py docstring)."
            ),
        }
    if not GRAPH_PATH.exists():
        raise HTTPException(
    ...
    if index is None:
        return {
            "ok": False,
            "message": (
                "Embedding index could not be built. "
                "Check that the model is cached (FASTEMBED_CACHE_PATH or ~/.cache/fastembed). "
                "See app/embeddings.py docstring for pre-seeding instructions."
            ),
        }
