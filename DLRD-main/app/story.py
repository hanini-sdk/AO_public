...
def _node_line(n: dict) -> str:
    ...
    base = f"- {name}: {summ}" if summ else f"- {name}"
    # Business labels from the variable dictionary, when present (capped). The
    # whole block is still passed through sanitize_for_enrichment by the caller.
    attrs = n.get("attributes")
    if isinstance(attrs, dict) and attrs:
        base += " (" + "; ".join(f"{k}: {v}" for k, v in list(attrs.items())[:3]) + ")"
    return base
...
def generate_story(graph: dict, llm: LLMClient, settings: Settings,
                   doc_context: dict | None = None) -> dict:
    ...
    content reasons — each section degrades to a deterministic narrative. When a
    document context store is supplied and carries a global narrative, it is
    incorporated as a clearly-labeled section; if it is absent (e.g. generated
    before the document phase ran) the story simply has less context.
    """
    ...
    # Document-sourced narrative (sidecar context store). Already redacted +
    # summarized upstream; here we only read it. No LLM call, no topology change.
    narrative = str((doc_context or {}).get("globalNarrative") or "").strip()
    if narrative:
        add("documents", "From project documents", narrative, [])
