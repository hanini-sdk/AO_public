...
# C1/C2 column granularity: cap how many used columns become nodes per table so a
# very wide table can't flood the canvas (columns are hidden by default anyway).
    ...
    *,
    ...
    if sql_parses:
        ...
        for tbl in sorted(cols_by_table):
            ...
            if not t_id:
                continue
            for col in sorted(cols_by_table[tbl])[:_MAX_COLUMNS_PER_TABLE]:
        ...
        for fp in sql_parses:
            ...
            for tbl in sorted(cr.used_columns):
                for col in sorted(c for c in cr.used_columns[tbl] if c)[:_MAX_COLUMNS_PER_TABLE]:
