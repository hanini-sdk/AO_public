...
const LearnReadingView = lazy(() => import("./components/LearnReadingView"));
...
function App() {
  // Local, single-user app served on 127.0.0.1 — the loopback bind IS the
  // security boundary, so the upstream shared-link token gate is unnecessary
  // here. Always render the dashboard with a constant token; the local backend
  // ignores the token query parameter. (Offline adaptation of the real dashboard.)
  return <Dashboard accessToken="local" />;
}
...
function Dashboard({ accessToken }: { accessToken: string }) {
  ...
  const setProjectContext = useDashboardStore((s) => s.setProjectContext);
  ...
  useEffect(() => {
    // Optional cached project story (Learn view). 404 until an analysis (or a
    // Learn-view regenerate) produces it; tolerated like meta.json.
    fetch(dataUrl("story.json", accessToken))
      .then((r) => (r.ok ? r.json() : null))
      .then((story) => {
        if (story && Array.isArray(story.sections)) setStory(story);
      })
      .catch(() => {});
  }, [setStory]);

  useEffect(() => {
    // Optional sidecar document context. 404 until the document phase fills it;
    // read lazily so the INFO panel can show "From project documents".
    fetch(dataUrl("project-context.json", accessToken))
      .then((r) => (r.ok ? r.json() : null))
      .then((ctx) => {
        if (ctx && typeof ctx === "object" && ctx.nodes) setProjectContext(ctx);
      })
      .catch(() => {});
  }, [setProjectContext]);
  ...
  return (
    // UI chrome is always English; the output language only affects generated
    // graph content (summaries/tours), which is data rendered as-is.
...
function DashboardContent({
  ...
  const learnViewOpen = useDashboardStore((s) => s.learnViewOpen);
  ...
  const toggleColumnsView = useDashboardStore((s) => s.toggleColumnsView);
  const clusterEnabled = useDashboardStore((s) => s.clusterEnabled);
  const toggleClusterize = useDashboardStore((s) => s.toggleClusterize);
  const hierarchyEnabled = useDashboardStore((s) => s.hierarchyEnabled);
  const toggleHierarchy = useDashboardStore((s) => s.toggleHierarchy);
  const missingView = useDashboardStore((s) => s.missingView);
  const toggleMissingView = useDashboardStore((s) => s.toggleMissingView);
    ...
    () => [
      ...
      // Navigation
      {
        ...
        action: () => {
          ...
          if (state.learnViewOpen) {
            state.closeLearnView();
            ...
            (state.graph?.layers?.length ?? 0) >= 2
          ) {
            // Only return to the overview when one exists (multi-layer). Single
            // -layer projects have no overview, so Esc falls through instead.
      ...
      <div className="flex items-center gap-1 p-2 border-b border-border-subtle bg-surface shrink-0">
        {(["info", "files", "chat"] as const).map((tab) => (
          <button
            ...
            disabled={tab === "chat" && !graph}
            className={`flex-1 px-2 py-1.5 rounded-md text-xs font-semibold uppercase tracking-wider transition-colors ${
              sidebarTab === tab
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-root text-text-primary noise-overlay">
          ...
          <div className="flex items-center gap-4 w-max">
            ...
            <div className="flex items-center gap-1">
              {(isKnowledgeGraph ? [
                ...
                // CODE and DATA map to node types this backend emits (files/
                // functions/classes, and SQL tables). The rest are future (v2)
                // node kinds, shown disabled with a "soon" marker.
                const isSoon =
                  !isKnowledgeGraph && cat.key !== "code" && cat.key !== "data";
                return (
                <button
                  key={cat.key}
                  onClick={isSoon ? undefined : () => toggleNodeTypeFilter(cat.key)}
                  disabled={isSoon}
                  className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded border transition-colors flex items-center gap-1.5 whitespace-nowrap ${
                    isSoon
                      ? "border-dashed border-border-subtle bg-transparent text-text-muted/50 cursor-not-allowed"
                  ...
                  title={isSoon ? t.common.comingSoon : `${nodeTypeFilters[cat.key] !== false ? "Hide" : "Show"} ${cat.label} nodes`}
                >
                  <span
                    className="w-2 h-2 rounded-full shrink-0"
                    style={{
                      backgroundColor: cat.color,
                      opacity: isSoon || nodeTypeFilters[cat.key] === false ? 0.3 : 1,
                    }}
                  />
                  {cat.label}
                  {isSoon && (
                    <span className="text-[8px] lowercase tracking-normal opacity-70">{t.common.soon}</span>
                  )}
                </button>
                );
              })}
            </div>
            {/* COL: render the used columns as nodes attached to their table.
                Off by default; a view-mode toggle, not a node-type filter.
                Columns live inside tables, so COL is only enabled when TABLE
                (the data node-type toggle) is on. */}
            {!isKnowledgeGraph && viewMode !== "domain" && (() => {
              const tableShown = nodeTypeFilters.data !== false;
              return (
                <button
                  type="button"
                  onClick={tableShown ? toggleColumnsView : undefined}
                  disabled={!tableShown}
                  title={tableShown ? t.detailLevel.columnsTitle : t.detailLevel.columnsNeedsTable}
                  className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded border transition-colors whitespace-nowrap ${
                    !tableShown
                      ? "border-dashed border-border-subtle bg-transparent text-text-muted/40 cursor-not-allowed"
                      : columnsView
                        ? "border-amber-500/50 bg-amber-500/10 text-amber-400"
                        : "border-border-medium bg-elevated text-text-muted hover:text-text-secondary"
                  }`}
                >
                  {t.detailLevel.columns}
                </button>
              );
            })()}
            {/* Clusterize: group the graph's nodes into folder/cluster boxes.
                Lives in the top bar so it stays reachable regardless of node
                selection or which side-panel tab is active. */}
            {!isKnowledgeGraph && viewMode !== "domain" && (
              <button
                type="button"
                onClick={toggleClusterize}
                title={t.fileExplorer.clusterizeTitle}
                className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded border transition-colors whitespace-nowrap ${
                  clusterEnabled
                    ? "border-amber-500/50 bg-amber-500/10 text-amber-400"
                    : "border-border-medium bg-elevated text-text-muted hover:text-text-secondary"
              ...
              >
                {t.fileExplorer.clusterize} {clusterEnabled ? "on" : "off"}
              </button>
            )}
            {/* Hierarchical (directory-tree) clustering: groups nodes by folder
                path. Mutually exclusive with Clusterize (the store enforces the
                radio behavior); off by default so the graph stays flat. */}
            {!isKnowledgeGraph && viewMode !== "domain" && (
              <button
                type="button"
                onClick={toggleHierarchy}
                title={t.fileExplorer.hierarchyTitle}
                className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded border transition-colors whitespace-nowrap ${
                  hierarchyEnabled
                    ? "border-amber-500/50 bg-amber-500/10 text-amber-400"
                    : "border-border-medium bg-elevated text-text-muted hover:text-text-secondary"
              ...
              >
                {t.fileExplorer.hierarchy} {hierarchyEnabled ? "on" : "off"}
              </button>
            )}
            {/* MISSING DATA: surface red placeholder nodes for scripts/SQL files
                referenced but not found in the project. Off by default; off = the
                graph is byte-for-byte identical to before the feature. */}
            {!isKnowledgeGraph && viewMode !== "domain" && (
              <button
                type="button"
                onClick={toggleMissingView}
                title={t.fileExplorer.missingDataTitle}
                className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded border transition-colors whitespace-nowrap ${
                  missingView
              ...
              >
                {t.fileExplorer.missingData} {missingView ? "on" : "off"}
              </button>
            )}
            <LayerLegend />
          </div>
      ...
      {/* Learn — project story reading view (lazy; generates/caches on first open). */}
      {learnViewOpen && (
        <Suspense fallback={null}>
          <LearnReadingView />
        </Suspense>
