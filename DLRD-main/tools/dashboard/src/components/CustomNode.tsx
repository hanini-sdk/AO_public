...
function CustomNodeComponent({
  ...
  const isMissing = knownType === "missing";
  ...
  // Missing-reference nodes get a red FILL + red border — deliberately distinct
  // from the Diff convention's red BORDER-only, so the two never read the same.
  const surfaceClass = isMissing
    ? "bg-[#3a1414] border border-[#ef4444]"
    : "bg-elevated border border-border-subtle";
  // A missing node is always an absent referenced FILE — a script or a SQL file
  // (the kind is baked into tags by the backend). Show that category in the chip.
  const missingKind = data.tags?.includes("sql-file") ? "sql file" : "script";
  const typeLabel = isMissing ? `missing ${missingKind}` : data.nodeType;
  const { t } = useI18n();
  ...
  return (
    <div
      className={`relative rounded-lg ${surfaceClass} ${extraClass} min-w-[180px] max-w-[220px] overflow-hidden transition-[box-shadow,outline,opacity,filter] duration-200 cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.3)]`}
