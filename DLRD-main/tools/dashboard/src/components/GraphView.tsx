...
function useLayerDetailTopology(): LayerDetailTopology & {
  ...
  const selectedNodeId = useDashboardStore((s) => s.selectedNodeId);
  const oversized = useDashboardStore((s) => s.oversized);
  ...
  const built = useMemo(() => {
    ...
    // Outside lineage mode a layer is required; and when oversized the focus
    // prompt is shown instead of the canvas, so skip building the (expensive)
    // full-layer topology entirely.
    if (!lineageMode && (!activeLayerId || oversized)) return null;
    ...
    // Derive containers + bucket edges. With clustering OFF (the default) the
    // layer renders flat: every node becomes its own ungrouped atom, which the
    // existing ungrouped pipeline lays out directly. Clustering ON restores the
    // folder/community container boxes.
    const { containers, ungrouped } = clusterEnabled
    ...
    const aggEdges: Edge[] = clusterEnabled
...
function GraphViewInner() {
  ...
  // Oversized graph: do not render the full canvas. Show the focus prompt until
  // the user picks a node to view its lineage (the only renderable path while
  // the graph is over the size threshold).
  if (oversized && !lineageActive) {
    return <FocusPrompt />;
  }
