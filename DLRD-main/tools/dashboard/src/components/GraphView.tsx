...
import { buildLineageIndex, traceLineage } from "../utils/lineage";
import type {
  GraphEdge,
  GraphNode,
  KnowledgeGraph,
...
import { deriveContainers, deriveTreeContainers } from "../utils/containers";
...
const NODE_TYPE_TO_CATEGORY: Record<NodeType, NodeCategory> = {
  ...
  table: "data", column: "data", endpoint: "data", schema: "data",
  domain: "domain", flow: "domain", step: "domain",
  article: "knowledge", entity: "knowledge", topic: "knowledge", claim: "knowledge", source: "knowledge",
  // "missing" is a render-only overlay; the layer-detail filter bypasses the
  // category check for it (visibility is the MISSING DATA toggle). Mapped here
  // only to satisfy the Record<NodeType> exhaustiveness.
  missing: "code",
} as const;

// Data-read edges: a reader reads FROM a source, so the data conceptually flows
// the OPPOSITE way to the source->target arrow. For these edge types we reverse
// the marching-ants flow animation (via the `rf-edge-reverse` CSS class) so the
// dashes travel in the real data direction. Animation only — endpoints,
// arrowheads, and topology are untouched. 'read'/'call' are defensive: the
// backend emits the canonical 'reads_from'/'calls' (and 'contains' table->column),
// but normalization aliases 'read'/'call' to those, so we cover both.
const REVERSE_FLOW_EDGE_TYPES = new Set(["reads_from", "read", "calls", "call"]);

// Structural containment edges (e.g. table -> column from the column-level
// granularity). They express "lives inside", not data flow, so they render as a
// plain static connector — solid stroke, never the marching-ants flow animation,
// no dashes — via the `rf-edge-contains` CSS class. This keeps containment
// visually distinct from the animated read/write flow edges.
const STRUCTURAL_EDGE_TYPES = new Set(["contains"]);

// Per-edge CSS class derived from the underlying graph edge type:
//   'contains'           -> rf-edge-contains (solid, static, undashed)
//   'reads_from'/'calls' -> rf-edge-reverse  (flow animation direction flipped)
//   anything else        -> none (default animated flow styling)
// The two sets are disjoint, so an edge receives at most one class.
function edgeFlowClass(edgeType: string | undefined): string | undefined {
  if (!edgeType) return undefined;
  if (STRUCTURAL_EDGE_TYPES.has(edgeType)) return "rf-edge-contains";
  if (REVERSE_FLOW_EDGE_TYPES.has(edgeType)) return "rf-edge-reverse";
  return undefined;
}

// An edge incident to a "missing reference" node renders red (rf-edge-missing,
// see index.css) when the MISSING DATA overlay is on. A missing edge is a
// calls/imports edge so edgeFlowClass is undefined for it, but we compose anyway.
function isMissingEdge(source: string, target: string, nodesById: Map<string, GraphNode>): boolean {
  return nodesById.get(target)?.type === "missing" || nodesById.get(source)?.type === "missing";
}
function edgeDisplayClass(edgeType: string | undefined, missing: boolean): string | undefined {
  const flow = edgeFlowClass(edgeType);
  if (!missing) return flow;
  return flow ? `${flow} rf-edge-missing` : "rf-edge-missing";
}
...
function useLayerDetailTopology(): LayerDetailTopology & {
  ...
  const clusterEnabled = useDashboardStore((s) => s.clusterEnabled);
  const hierarchyEnabled = useDashboardStore((s) => s.hierarchyEnabled);
  const columnsView = useDashboardStore((s) => s.columnsView);
  const missingView = useDashboardStore((s) => s.missingView);
  const lineageActive = useDashboardStore((s) => s.lineageActive);
  const lineageDirection = useDashboardStore((s) => s.lineageDirection);
  const lineageLimit = useDashboardStore((s) => s.lineageLimit);
  const selectedNodeId = useDashboardStore((s) => s.selectedNodeId);

  // Memoized per graph; drives the exclusive lineage universe below.
  const lineageIndex = useMemo(
    () => (graph ? buildLineageIndex(graph) : null),
    [graph],
  );
  ...
  const built = useMemo(() => {
    ...
    // ── Lineage focus: an EXCLUSIVE, cross-layer node universe ──────────────
    // When a lineage trace is active, the rendered universe is the traced
    // lineage set (transitive upstream/downstream across ALL layers), NOT the
    // active layer. Layer scoping, the persona filter, and the node-type
    // category filter are all bypassed — the user asked to see this whole
    // lineage. Only the COL gate applies (columns hidden unless columnsView),
    // and the root is always kept. The lineage budget (lineageLimit) caps the
    // set, so even a giant lineage never mounts more than ~budget nodes. Edge
    // bucketing, clustering, and ELK layout below are reused unchanged.
    const lineageMode = lineageActive && !!selectedNodeId && !!lineageIndex;
    // Outside lineage mode a layer is required to scope the build.
    if (!lineageMode && !activeLayerId) return null;

    let filteredGraphNodes: GraphNode[];
    let filteredNodeIds: Set<string>;
    let filteredGraphEdges: GraphEdge[];

    if (lineageMode) {
      const universe = traceLineage(lineageIndex!, selectedNodeId!, lineageDirection, lineageLimit).nodeIds;
      filteredGraphNodes = graph.nodes.filter(
        (n) => universe.has(n.id) && (n.id === selectedNodeId || columnsView || n.type !== "column"),
      );
      filteredNodeIds = new Set(filteredGraphNodes.map((n) => n.id));
      filteredGraphEdges = graph.edges.filter(
      ...
      // parent file is in this layer, joined via "contains" edges. File view:
      // file nodes only. Class view: + class nodes (+ functions when toggled).
      ...
      // Column nodes are the finest data granularity — hidden until the user
      // turns on the top-bar COLUMNS toggle, so the default view is unchanged.
      if (columnsView) allVisibleTypes.add("column");
      // Missing-reference overlay: when MISSING DATA is on, allow the "missing"
      // type through and pull the layerless placeholder nodes referenced by a
      // node in this layer into the layer's node set. OFF -> skipped, so the
      // view is byte-for-byte identical to before that feature.
      if (missingView) {
        allVisibleTypes.add("missing");
        for (const edge of graph.edges) {
          if (!expandedLayerNodeIds.has(edge.source)) continue;
          const tgt = nodesById.get(edge.target);
          if (tgt && tgt.type === "missing") expandedLayerNodeIds.add(edge.target);
        }
      }
      ...
      nodes = nodes.filter((n) => {
        if (n.type === "missing") return true;  // overlay — gated by MISSING DATA, not categories
      ...
      let gedges = graph.edges.filter((e) => ids.has(e.source) && ids.has(e.target));
      ...
      if (focusNodeId && ids.has(focusNodeId)) {
        ...
        nodes = nodes.filter((n) => focusNeighborIds.has(n.id));
        ids = new Set(nodes.map((n) => n.id));
        gedges = gedges.filter((e) => ids.has(e.source) && ids.has(e.target));
      }

      filteredGraphNodes = nodes;
      filteredNodeIds = ids;
      filteredGraphEdges = gedges;
    }

    // Derive containers + bucket edges. With BOTH clustering modes OFF (the
    // default) the layer renders flat: every node becomes its own ungrouped
    // atom, which the existing ungrouped pipeline lays out directly. Clusterize
    // restores the folder/community boxes; the hierarchical mode (mutually
    // exclusive with it) groups by full directory-tree path instead.
    const clusteringOn = clusterEnabled || hierarchyEnabled;
    const { containers, ungrouped } = hierarchyEnabled
      ? deriveTreeContainers(filteredGraphNodes)
      : clusterEnabled
    ...
    // Display edges. Clustered: aggregated cross-atom edges (count label,
    // log-scaled stroke); diffMode dims them (no per-edge diff data at this
    // stage). Flat (clustering OFF): render each real edge directly so its
    // type + description label (op sequence, "feeds via …", "runs") survive —
    // aggregation would collapse them to a bare count. The original edge type
    // rides along in `data.edgeType` for downstream styling.
    const aggEdges: Edge[] = clusteringOn
      ? interContainerAggregated.map((agg, i) => {
      ...
      : filteredGraphEdges.map((e, i) => ({
          id: `fe-${i}`,
          source: e.source,
          target: e.target,
          label: e.description ?? e.type,
          data: { edgeType: e.type },
          className: edgeDisplayClass(e.type, isMissingEdge(e.source, e.target, nodesById)),
          style: diffMode
            ? { stroke: "rgba(212,165,116,0.08)", strokeWidth: 1 }
    ...
    // Portal nodes for connected external layers. None in lineage mode — the
    // lineage already spans layers inline, so cross-layer portals are moot.
    const portals = lineageMode ? [] : computePortals(graph, activeLayerId);
...
function useLayerDetailGraph() {
  ...
  // Lineage trace: index the graph once per graph, then walk the selected
  // table's transitive upstream/downstream. `lineageResult` is null unless a
  // trace is active — callers fall back to the 1-hop selection highlight.
  const lineageIndex = useMemo(
    () => (graph ? buildLineageIndex(graph) : null),
    [graph],
  );
  const lineageResult = useMemo(
    () =>
      lineageActive && selectedNodeId && lineageIndex
        ? traceLineage(lineageIndex, selectedNodeId, lineageDirection, lineageLimit)
        : null,
    [lineageActive, selectedNodeId, lineageIndex, lineageDirection, lineageLimit],
  );
  ...
  const selectionContainerIds = useMemo(() => {
    const s = new Set<string>();
    // Lineage trace: emphasize every container that holds a traced node, so
    // collapsed folders on the lineage path still stand out.
    if (lineageResult) {
      for (const id of lineageResult.nodeIds) {
        const cid = topo.nodeToContainer.get(id);
        if (cid && cid !== id) s.add(cid);
      }
      return s;
    ...
    return s;
  }, [lineageResult, selectedNodeId, topo.filteredEdges, topo.nodeToContainer]);
  ...
  const nodes = useMemo(() => {
    ...
    // Build the highlight set: the full transitive lineage when a trace is
    // active, otherwise the selected node's 1-hop neighborhood.
    ...
    const highlightSet = lineageResult ? lineageResult.nodeIds : neighborNodeIds;
    ...
    return combined.map((node) => {
      ...
      const isNeighbor = hasSelection && highlightSet.has(node.id) && !isSelected;
      const isSelectionFaded = hasSelection && !highlightSet.has(node.id);
    ...
    for (const e of topo.edges) {
      ...
      for (const m of matching) {
        ...
        out.push({
          ...
          data: { edgeType: m.type },
          className: edgeDisplayClass(m.type, isMissingEdge(m.source, m.target, nodesById)),
  ...
  const edges = useMemo(() => {
    ...
    // Apply selection-based edge styling on top of topology edges. With a
    // lineage trace active, an edge is "on the path" when BOTH endpoints are in
    // the traced set; otherwise it's any edge incident to the selection.
    const lineageSet = lineageResult ? lineageResult.nodeIds : null;
    return base.map((edge) => {
      // Don't restyle diff-impacted or portal edges
      if ((edge.style as Record<string, unknown>)?.strokeDasharray) return edge;
      const isSelectedEdge = lineageSet
        ? lineageSet.has(String(edge.source)) && lineageSet.has(String(edge.target))
  ...
  }, [expandedEdges, topo.portalEdges, selectedNodeId, lineageResult]);
  ...
  return {
    ...
    lineageInfo: lineageResult
      ? { total: lineageResult.total, truncated: lineageResult.truncated }
      : null,
  };
}
...
function GraphViewInner() {
  ...
  const selectedNodeId = useDashboardStore((s) => s.selectedNodeId);
  const lineageDirection = useDashboardStore((s) => s.lineageDirection);
  const setLineageDirection = useDashboardStore((s) => s.setLineageDirection);
  const showMoreLineage = useDashboardStore((s) => s.showMoreLineage);
  const toggleLineageTrace = useDashboardStore((s) => s.toggleLineageTrace);
  const { preset } = useTheme();

  const overviewGraph = useOverviewGraph();
  const detailGraph = useLayerDetailGraph();
  const lineageInfo = detailGraph.lineageInfo;
  ...
  // Center the viewport on the focal node when a lineage trace is activated.
  // Clicking "Trace Lineage" flips lineageActive but does NOT change
  // selectedNodeId (the node is already selected), so the selection-driven fit
  // never re-fires — without this the viewport stays on the pre-trace framing
  // or drifts to an edge of the re-laid-out subgraph instead of the node the
  // user traced from. Layout is async, so (like pendingFit) we mark a pending
  // center when the trace turns on and run it once the focal node has a
  // position in the freshly built lineage universe.
  const pendingTraceCenterRef = useRef(false);
  const prevLineageActiveRef = useRef(lineageActive);
  useEffect(() => {
    if (lineageActive && !prevLineageActiveRef.current) {
      pendingTraceCenterRef.current = true;
    }
    prevLineageActiveRef.current = lineageActive;
  }, [lineageActive]);

  useEffect(() => {
    if (!pendingTraceCenterRef.current) return;
    if (!selectedNodeId) {
      pendingTraceCenterRef.current = false;
      return;
    }
    // Wait until the focal node is actually laid out in the new universe.
    if (!nodes.some((n) => n.id === selectedNodeId)) return;
    pendingTraceCenterRef.current = false;
    const raf = requestAnimationFrame(() => {
      // fitView on the single focal node centers + zooms it into view — the
      // same primitive SelectedNodeFitView uses for search/selection centering.
      fitView({
        nodes: [{ id: selectedNodeId }],
        duration: 500,
        padding: 0.3,
    ...
    return () => cancelAnimationFrame(raf);
  }, [nodes, selectedNodeId, lineageActive, fitView]);
  ...
  return (
    <div className="h-full w-full relative">
      ...
      {lineageActive && navigationLevel === "layer-detail" && lineageInfo && (
        <div className="absolute top-14 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2 px-3 py-1.5 rounded-full bg-elevated border border-gold/30 shadow-lg">
          <span className="text-gold text-xs font-semibold tracking-wider uppercase">
            Lineage
          </span>
          <div className="flex items-center rounded-full bg-root/60 p-0.5">
            {(["up", "both", "down"] as const).map((dir) => (
              <button
                key={dir}
                onClick={() => setLineageDirection(dir)}
                title={
                  dir === "up"
                    ? "Upstream — where this data comes from"
                    : dir === "down"
                      ? "Downstream — where this data propagates"
                      : "Both directions"
                }
                className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide transition-colors ${
                  lineageDirection === dir
                    ? "bg-gold/20 text-gold"
                    : "text-text-muted hover:text-text-secondary"
                }`}
              >
                {dir === "up" ? "Upstream" : dir === "down" ? "Downstream" : "Both"}
              </button>
            ))}
          </div>
          <span className="text-text-muted text-[11px] tabular-nums">
            {lineageInfo.total} node{lineageInfo.total === 1 ? "" : "s"}
          </span>
          {lineageInfo.truncated && (
            <button
              onClick={showMoreLineage}
              className="text-[10px] font-semibold uppercase tracking-wider text-accent hover:text-accent-bright transition-colors"
              title="Trace was capped — reveal more lineage"
            >
              Show more
            </button>
          )}
          <button
            onClick={toggleLineageTrace}
            className="text-text-muted hover:text-gold transition-colors text-sm leading-none"
            title="Exit lineage trace"
          >
            &times;
          </button>
        </div>
