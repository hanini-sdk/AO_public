...
const typeBadgeColors: Record<NodeType, string> = {
  ...
  column: "text-node-schema border border-node-schema/30 bg-node-schema/10",
  ...
  missing: "text-[#ef4444] border border-[#ef4444]/40 bg-[#ef4444]/10",
};
...
export default function NodeInfo() {
  ...
  const projectContext = useDashboardStore((s) => s.projectContext);
  ...
  // Resolve child nodes. Columns are split off into their own COLUMNS section
  // (below) so they don't clutter the generic "defined in this file" list.
  const allChildNodes = childEdges
    .map((e) => activeGraph?.nodes.find((n) => n.id === e.target))
    .filter((n): n is GraphNode => n !== undefined);
  const childNodes = allChildNodes.filter((n) => n.type !== "column");
  const columnNodes = allChildNodes.filter((n) => n.type === "column");
  ...
  return (
    <div className="h-full w-full overflow-auto p-5 animate-fade-slide-in">
      ...
      {/* Business labels/descriptions from the project variable dictionary. */}
      {node.attributes && Object.keys(node.attributes).length > 0 && (
        <div className="mb-4">
          <h3 className="text-[11px] font-semibold text-accent uppercase tracking-wider mb-2">
            Attributes
          </h3>
          <dl className="text-xs rounded-lg border border-border-subtle bg-elevated/60 p-3 space-y-1.5">
            {Object.entries(node.attributes).map(([key, value]) => (
              <div key={key} className="flex gap-2">
                <dt className="font-medium text-text-muted shrink-0 capitalize">
                  {key.replace(/_/g, " ")}
                </dt>
                <dd className="text-text-secondary break-words min-w-0">{value}</dd>
              </div>
            ))}
          </dl>
        </div>
      ...
      {/* Document-sourced context (sidecar store). Visually DISTINCT from the
          parsed facts above and from the dictionary's business labels: a dashed
          amber panel, marked approximate, so the user knows it is summarized
          from project documents and may not be exact. */}
      {(projectContext?.nodes?.[node.id]?.length ?? 0) > 0 && (
        <div className="mb-4">
          <h3 className="text-[11px] font-semibold text-amber-400/90 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <span>From project documents</span>
            <span className="text-[9px] font-normal normal-case text-text-muted/70 tracking-normal">
              (summarized · approximate)
            </span>
          </h3>
          <div className="space-y-2">
            {projectContext!.nodes[node.id].map((snip, i) => (
              <div
                key={`${snip.docId}-${i}`}
                className="text-xs rounded-lg border border-dashed border-amber-500/40 bg-amber-500/5 p-3"
              >
                <div className="font-mono text-[10px] text-amber-400/80 mb-1 truncate" title={snip.label}>
                  {snip.label}
                </div>
                <p className="text-text-secondary leading-relaxed break-words">{snip.snippet}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {(node.type === "table" || node.type === "column") && (
        <button
          ...
          className={`w-full mb-4 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors ${
            lineageActive
              ? "bg-gold/20 text-gold border border-gold/40"
              : "bg-elevated text-text-secondary border border-border-subtle hover:text-gold hover:border-gold/30"
          }`}
          title={
            node.type === "column"
              ? "Highlight this column's full upstream and downstream lineage"
              : "Highlight this table's full upstream and downstream lineage"
          }
        >
          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M4 7h16M4 12h16M4 17h10" />
          </svg>
          {lineageActive ? "Tracing lineage" : "Trace lineage"}
        </button>
      )}
            ...
            {otherConnections.map((edge, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* For edges carrying a description (file<->table op sequence,
                      or a "feeds via …" edge) show that instead of the raw
                      reads_from/writes_to label; the arrow already conveys
                      direction. Other edge types keep their directional label. */}
                  {edge.description ? (
                    <span
                      className="font-mono text-[10px] text-gold/90 bg-gold/10 rounded px-1.5 py-0.5 shrink-0"
      ...
      {/* COLUMNS — used columns of a selected SQL table. Selecting one re-roots
          the selection on that column node and starts a column-level lineage
          trace (reusing the same highlight/dim machinery as table lineage). */}
      {node.type === "table" && columnNodes.length > 0 && (
        <div className="mb-4">
          <h3 className="text-[11px] font-semibold text-gold uppercase tracking-wider mb-2">
            {t.nodeInfo.columns} ({columnNodes.length})
          </h3>
          <div className="space-y-1">
            {columnNodes.map((col) => (
              <button
                ...
                title="Trace this column's upstream and downstream lineage"
                className="w-full text-left text-xs bg-elevated rounded-lg px-3 py-2 border border-border-subtle cursor-pointer hover:border-gold/40 hover:bg-gold/5 transition-colors flex items-center gap-2"
              >
                <span className="text-gold font-mono shrink-0">{"›"}</span>
                <span className="font-mono text-text-primary truncate">{col.name}</span>
              </button>
            ))}
          </div>
        </div>
