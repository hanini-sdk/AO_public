...
interface DashboardStore {
  ...
  // Hierarchical (directory-tree) clustering: group nodes by their position in
  // the project's folder tree (one container per directory path). Mutually
  // exclusive with clusterEnabled — enabling one disables the other (radio).
  // OFF by default, so the graph renders flat until the user opts in.
  ...
  // Sidecar document context (data/project_context.json), loaded lazily. Read by
  // the INFO panel ("From project documents"); absent until the doc phase fills
  // it. Never affects topology.
...
export const useDashboardStore = create<DashboardStore>()((set, get) => ({
  ...
  toggleClusterize: () =>
    set((state) => ({
      ...
      // Mutually exclusive with hierarchical clustering: turning Clusterize ON
      // forces the tree mode OFF (radio behavior). Turning it OFF leaves tree
      // mode off too, so the graph falls back to flat.
  ...
  toggleHierarchy: () =>
    set((state) => ({
      ...
      // Radio behavior: enabling the directory-tree grouping disables the
      // folder/community Clusterize, and vice versa. Both off => flat graph.
      clusterEnabled: false,
      // Same layout-affecting reset as Clusterize; selection/trace/focus kept.
      containerLayoutCache: new Map(),
      containerSizeMemory: new Map(),
      expandedContainers: new Set(),
      pendingFocusContainer: null,
