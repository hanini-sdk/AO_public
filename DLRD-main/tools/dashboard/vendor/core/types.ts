// Node types (23 total: 22 + 1 "missing" reference-overlay type)
export type NodeType =
  | "file" | "function" | "class" | "module" | "concept"
  | "config" | "document" | "service" | "table" | "column" | "endpoint"
  | "pipeline" | "schema" | "resource" | "missing"
  | "domain" | "flow" | "step"
...
// GraphNode with 22 types: 5 code + 9 non-code + 3 domain + 5 knowledge
export interface GraphNode {
  ...
  // Business labels/descriptions attached deterministically from a project
  // variable dictionary (CSV/Excel). Sanitized header -> value. Display only.
  attributes?: Record<string, string>;
  domainMeta?: DomainMeta;
  knowledgeMeta?: KnowledgeMeta;
}

// Sidecar document-context store (data/project_context.json). Holds prose
// summaries derived from project documents; it NEVER adds graph nodes/edges and
// is read lazily by the INFO panel and chat. Document-sourced and approximate.
...
export interface ProjectContext {
  ...
  nodes: Record<string, DocSnippet[]>;
}
