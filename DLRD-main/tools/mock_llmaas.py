...
async def chat_completions(req: Request):
    ...
    if "Reply with the single word: OK" in joined:
        ...
        # Graph-aware chat request: the system prompt embeds the knowledge-graph
        # context. Extract a few node ids + the user's question to build a
        # deterministic, citation-bearing reply (English only).
        question = last.strip()
        ...
        for ref in node_refs:
            ...
            if len(top_nodes) == 3:
                break
        name_map: dict = {}
        for mm in re.finditer(r"\[\[([^\]]+)\]\]\s+\w+\s+«([^»]+)»", joined):
            name_map[mm.group(1)] = mm.group(2)
        citations = [
            f"**{name_map.get(nid, nid.split(':')[-1])}** ([[{nid}]])" for nid in top_nodes
        ]
        if citations:
            content = (
                f"[mock response] Based on the knowledge graph, your question "
                f'"{question}" relates to: {", ".join(citations)}.\n\n'
                "These nodes are part of the sample project. "
                "Replace the mock with a real model to get accurate answers."
            )
        else:
            content = (
